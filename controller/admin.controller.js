...
const getAllBasicUsers = async (req, res) => {
  try {
    ...
    const basicUsers = users.filter(
      (user) =>
        user.package.toLocaleLowerCase() === "arbitrage" &&
...
const getAllPremiumUsers = async (req, res) => {
  try {
    ...
    const premiumUsers = users.filter(
      (user) =>
        user.package.toLocaleLowerCase() === "high frequency" &&
        user.role.toLocaleLowerCase() != "admin"
    );
    res.status(200).json({
      success: true,
...
const getAllAdvanceUsers = async (req, res) => {
  try {
    const users = await userModel.find();
    const advanceUsers = users.filter(
      (user) =>
        user.package.toLocaleLowerCase() === "guy's" &&
        user.role.toLocaleLowerCase() != "admin"
    );
    res.status(200).json({
      success: true,
